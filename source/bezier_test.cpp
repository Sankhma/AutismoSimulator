...
int main(){
    std::cout << "-============ Bezier2 test =============-" << std::endl;
	{
        ...
        for(int i=0; i <= steps; i++){
            Vector2<double> temp = Bezier2::GenerateVertex(b0, i / double(steps));
            std::cout << "[ " << temp.x << ", " << temp.y << " ]" << std::endl;
        }

        std::cout << "Bezier2 for 3 points, steps=20" << std::endl;
        for(int i=0; i <= steps; i++){
            Vector2<double> temp = Bezier2::GenerateVertex(b1, i / double(steps));
            std::cout << "[ " << temp.x << ", " << temp.y << " ]" << std::endl;
        }

        std::cout << "Bezier2 for 4 points, steps=20" << std::endl;
        for(int i=0; i <= steps; i++){
            Vector2<double> temp = Bezier2::GenerateVertex(b2, i / double(steps));
            std::cout << "[ " << temp.x << ", " << temp.y << " ]" << std::endl;
        }
	}
}